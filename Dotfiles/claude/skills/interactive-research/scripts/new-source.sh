...
dir=$1; slug=$2
command -v jq   >/dev/null || die "jq not found — install it (brew install jq / scoop install jq)"
command -v curl >/dev/null || die "curl not found"

slug_re='^[A-F]-[a-z0-9]+(-[a-z0-9]+)*-(19|20)[0-9]{2}$'
[[ $slug =~ $slug_re ]] || die "slug '$slug' must be <A-F>-<lower-kebab>-<YYYY>, e.g. B-moeller-spa-2025"
...
[[ $(field url) =~ ^https?:// ]] || die "url must be http(s)://…"
cert_re='^([+]1:SURE|-0:SUSPECT|-1:GUESS|-2:WONDER)$'
for k in grading-certainty relevance-certainty; do
   [[ $(field "$k") =~ $cert_re ]] || die "$k must be one of +1:SURE / -0:SUSPECT / -1:GUESS / -2:WONDER"
done
[[ $(field graded-by) =~ ^(subagent|top-level-agent|human)$ ]] || die "graded-by must be subagent / top-level-agent / human"
[[ $(field published) =~ ^[0-9]{4}(-[0-9]{2}){0,2}$ ]] || die "published must be YYYY, YYYY-MM, or YYYY-MM-DD"
...
if   command -v sha256sum >/dev/null; then sha=$(sha256sum "$artifact" | cut -d' ' -f1)
elif command -v shasum    >/dev/null; then sha=$(shasum -a 256 "$artifact" | cut -d' ' -f1)
