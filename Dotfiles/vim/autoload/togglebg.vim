...
if exists('g:loaded_togglebg') || &compatible || v:version < 700
  finish
endif
...
function! togglebg#map(activation) abort
   try
...
function! s:complete_background(ArgLead, CmdLine, CursorPos) abort
...
function! togglebg#colorscheme(colo_name, ...) abort
   call <SID>find_links()
...
function! s:restore_links() abort " {{{1
